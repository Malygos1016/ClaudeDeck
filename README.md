...
**pi agent**(`~/.pi/agent/sessions` JSONL)与 **Crush**(经 `%LOCALAPPDATA%\crush\projects.json`
名册发现各项目的 `.crush\crush.db`,只读)。均支持浏览/中文搜索/聊天视图/导出;
一键拉起:codex 用 `codex resume`,pi 用 `pi --session`,crush 无命令行级 resume,
在项目目录打开 TUI 后选择。归档与实时看板仍为 Claude 专属。
